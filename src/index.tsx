...
function ShareSecretAction() {
  async function handleSubmit(values: { secret: string; expireView: number; expireDays: number }) {
    if (!values.secret) {
      showToast({
    ...
    try {
      const { body } = await got.post("https://api.doppler.com/v1/share/secrets/plain", {
        json: {
          ...
          expire_views: values.expireView,
      ...
      toast.message = "The secret has been shared and copied to your clipboard.";
    } catch (error) {
      toast.style = Toast.Style.Failure;
      toast.title = "Failed to share secret";
